...
class LoadStreams:
    ...
    def update(self, i, cap, stream):
        ...
        if is_ros:
            while True:
                # while not stream.get_queue_size() > 0:
                #     time.sleep(1)
                # print("KREKHAAAR", stream.get_queue_size())
    ...
    def __next__(self):
        ...
        if isinstance(self.sources[0], image_converter):
            ...
            if self.sources[0].sim_reset_queue.qsize() > 0:
                while(self.sources[0].sim_reset_queue.qsize() > 0):
