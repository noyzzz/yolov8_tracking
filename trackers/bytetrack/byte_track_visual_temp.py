...
class BYTETracker(object):
    ...
    def update(self, dets, frame_img):
        ...
        if len(detections) > 0:
            print("detections[0]", detections[0]._tlwh)
            print(frame_img.shape)
        x, y, w, h = detections[0]._tlwh
        x1 = int(x - w/2)
        y1 = int(y - h/2)
        x2 = int(x + w/2)
        y2 = int(y + h/2)
        ...
        print("strack_pool.shape", np.asarray(strack_pool).shape)
        print("detections.shape", np.asarray(detections).shape)
        #if not self.args.mot20:
        dists = matching.fuse_score(dists, detections)
        print("REAL DISTS", dists)
