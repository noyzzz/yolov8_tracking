...
from yolov8.ultralytics.yolo.utils import (
    DEFAULT_CFG,
    LOGGER,
    SETTINGS,
    callbacks,
...
from yolov8.ultralytics.yolo.utils.checks import (
    check_file,
    check_imgsz,
    check_imshow,
    print_args,
...
from yolov8.ultralytics.yolo.utils.ops import (
    Profile,
    non_max_suppression,
    scale_boxes,
    process_mask,
...
def play_bag(is_ros_bag):
    if is_ros_bag:
        player_state = subprocess.Popen(
            [
                "rosbag",
                "play",
                "/home/rosen/tracking_catkin_ws/src/my_tracker/bags/carla_stationary_obj_slow.bag",
            ]
        )
...
def run(
    ...
    for frame_idx, batch in enumerate(dataset):
        ...
        with dt[2]:
            if is_seg:
                masks = []
                nms_dets_list = non_max_suppression(
                    preds[0],
                    conf_thres,
                    iou_thres,
                    classes,
        ...
        for i, det in enumerate(nms_dets_list):  # detections per image
            ...
            with dt[3]:
                ...
                if extra_output["gt"] is not None:
                    ...
                    if len(modified_gt_list) and output_random < 0.6:
                        ...
                        modified_gt_list = add_noise2tensor(
                            modified_gt_list, [im0.shape[0], im0.shape[1]], 0.02
                        )
                    else:
                ...
                if (
                    ...
                    and hasattr(tracker_list[i], "use_odometry")
                    and tracker_list[i].use_depth
                    and tracker_list[i].use_odometry
                ):
                    if "kitti" in source:
                        odom = dict()
                        odom["header"] = "kitti"
            ...
            for j, (output) in enumerate(outputs[i]):
                ...
                if save_vid or save_crop or show_vid:  # Add bbox/seg to image
                    ...
                    intersection = get_intersection(
                        bbox, [0, 0, im0.shape[1], im0.shape[0]]
                    )
                    ...
                    if save_crop:
                        ...
                        if c == -1:
                            c = 1  # if no detection just save with class 1 for visualization
                        save_one_box(
                            np.array(bbox, dtype=np.int16),
                            imc,
                            file=save_dir
                            / "crops"
                            / txt_file_name
            ...
            if (
                ros_package == "1"
            ):  # it means that the image_detection message type is being generated and published
                from my_tracker.msg import ImageDetectionMessage
                ...
                if outputs[0] is not None and len(outputs[0]) > 0:
                    ...
                    tracks_object_bbs = outputs[0][
                        :, :5
                    ]  # track_id_size x (x1, y1, x2, y2, track_id)
            ...
            if show_vid:
                ...
                # im0 = test_kitti_odom(im0, dataset.calib.T_imu_cam, dataset.oxts[frame_idx].packet)
                # im0 = test_kitti_depth(im0, depth_image)
...
def parse_opt():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=Path,
        default=WEIGHTS / "yolov8s-seg.pt",
    ...
    parser.add_argument(
        "--ros-package", type=str, default="0", help="is this running in a ros package"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="eval",
        help='get detection from "yolo" or from "eval" ground truth',
    )
    parser.add_argument(
        "--use-depth", type=int, default=0, help="use emap (depth module) for tracking"
    )
    parser.add_argument(
        "--use-odometry",
        type=int,
